...
// Maps DID resolution error type keys to HTTP status codes per:
...
// Maps error type keys to W3C DID namespace URI per spec.
// The test suite expects error.type to be a full URI.
const ERROR_TYPE_URI = new Map([
  ['invalidDid', 'https://www.w3.org/ns/did#INVALID_DID'],
  ...
  ['deactivated', 'https://www.w3.org/ns/did#DEACTIVATED'],
  ['methodNotSupported', 'https://www.w3.org/ns/did#METHOD_NOT_SUPPORTED'],
  ['internalError', 'https://www.w3.org/ns/did#INTERNAL_ERROR']
]);
...
export function makeErrorObject(errorType) {
  return {type: ERROR_TYPE_URI.get(errorType) ??
