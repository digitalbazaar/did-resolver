...
export function classifyError(e, mode = 'resolution') {
  ...
  // TypeError from drivers signals malformed input, not a network failure.
  ...
  // throws a plain Error with no structured code. A future did-io improvement
  // would throw `err.name = 'MethodNotSupportedError'` so this can be
  // replaced with `e.name === 'MethodNotSupportedError'`.
  ...
  // did:web resolves by fetching a URL; any connection error means not found.
  if(!e.status) {
    return 'notFound';
  }
