 ...
 * 2. Structured: e.name === 'NotSupportedError' (no driver for DID method).
 * 3. Structured: e instanceof TypeError (bad-input errors from drivers).
 *
...
export function classifyError(e, mode = 'resolution') {
  // http-client sets e.status from the HTTP response status code. Any 4xx
  // from the remote host (404, 401, 403, 429, ...) means the document could
  // not be retrieved — the DID itself may still be perfectly valid, so
  // report notFound rather than invalidDid.
  if(e.status >= 400 && e.status < 500) {
    return 'notFound';
  }

  // did-io's CachedResolver sets e.name = 'NotSupportedError' when no driver
  // is registered for a DID method (did-io >= 2.2.0).
